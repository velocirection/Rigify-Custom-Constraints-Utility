...
READ_STATE_COLLECTING_HEADER                 = 9;  # get header info
READ_STATE_FINALIZE_CONSTRAINT               = 7;  # add the constraint
READ_STATE_FINALIZE_DRIVER                   = 8;  # add the constraint
READ_STATE_FINALIZE_HEADER                   = 10; # get header info

# @TODO: make it so rig target is easier to change by having the value in the header (and replace rig target in write function with "@RIG" or something)
...
def read_generated_rig_constraint_link_file(armature):    
    ...
    for line in f:
        ...
        if '@BEGIN' in cLine:
            if 'HEADER' in cLine:
                readState = READ_STATE_COLLECTING_HEADER
        ...
        if '@END' in cLine:
            if 'HEADER' in cLine:
                readState = READ_STATE_FINALIZE_HEADER
        ...
        if readState == READ_STATE_COLLECTING_HEADER:
            if '@BEGIN' in cLine:
                print(" ")
            elif '@END' in cLine:
                print(" ")
