...
if release_mode:
    if "bpy" in locals():
        ...
        if "rccu_setup_bbones" in locals():
            importlib.reload(rccu_setup_bbones)
    ...
    from .rccu_setup_bbones             import ( setup_bbones)
...
class rccuSetBBones(bpy.types.Operator):
    bl_idname = 'scene.rccusetbbones'
    bl_label = '#RCCU#SETBBONES'
    bl_description = 'Copies the bbone settings of \'ORG-\' bbones to the \'DEF-\' bones if the bone name contains \'|BB|\' (operates on \'Target Rig\''
    def execute(self, context):
        scene = context.scene
        print('>rccuSetBBones| Called...')
        setup_bbones(scene.metarig_object.data.rigify_target_rig.data)
        '''
        if scene.metarig_object != None:
            scene.metarig_object.select_set(True)
            bpy.context.view_layer.objects.active = scene.metarig_object
...
class RCCU_PT_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        box.operator(operator='scene.rccusetbbones',       text='Setup BBones')
        #box.row()
...
def register():
    ...
    bpy.utils.register_class(rccuSetBBones)
...
def unregister():
    ...
    bpy.utils.unregister_class(rccuSetBBones)
